...
export default function ProjectCard({ project }: Props) {
  ...
  // 🕐 Ładowanie źródła dopiero po pojawieniu się w viewport
  useEffect(() => {
    if (isMobile && isVisible && !videoSrc) {
      setVideoSrc(project.video); // ustaw src dopiero teraz
    }
  }, [isMobile, isVisible, project.video, videoSrc]);

  // ▶️ Auto-play tylko po ustawieniu src i widoczności
  useEffect(() => {
    if (isMobile && isVisible && videoRef.current && videoSrc) {
      videoRef.current.play().catch(() => {});
    }
  }, [isMobile, isVisible, videoSrc]);
